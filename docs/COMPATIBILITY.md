...
Node 22 is intentional. Next.js 16 requires Node >=20.9, while the current `pgvector` Node package and the current OpenAI Node SDK require Node >=22. Choosing Node 22 gives the broadest conservative overlap across the planned base stack.
...
- If a new package cannot support Node 22 + React 19.2 + Next 16.2, prefer replacing that package over downgrading the whole application unless it is strategically essential.
